...
class MyAuctionAdapter(private val listProduct: ArrayList<AuctionItem>): RecyclerView.Adapter<MyAuctionAdapter.MyAuctionViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyAuctionViewHolder {
        ...
        return MyAuctionViewHolder(view)
    }

    override fun onBindViewHolder(holder: MyAuctionViewHolder, position: Int) {
        val (owner, title, description, photoUrl, category, startingPrice, buyoutPrice, currentPrice) = listProduct[position]
        holder.tvOwner.text = "owner"
        holder.tvTitle.text = "title"
//        holder.imgPhoto.setImageResource(photoUrl.toInt())
        holder.tvCategory.text = "category"
        holder.tvBidPrice.text = 123.toString()
        holder.tvBuyOut.text = 123.toString()
    }
